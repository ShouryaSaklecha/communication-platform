...
from tests.routes_test import URL_channel_messages, URL_dm_messages, URL_message_pin
...
def test_INPUT_ERROR_user_not_in_channel(three_messages, third_user_flask):
    response = requests.post(URL_message_pin, json={"token": third_user_flask['token'], 
...
def test_INPUT_ERROR_message_already_pinned(three_messages):
    response = requests.post(URL_message_pin, json={"token": three_messages['token'], 
    "message_id" : three_messages['message1_ID']})
    response_1 = requests.post(URL_message_pin, json={"token": three_messages['token'], 
    "message_id" : three_messages['message1_ID']})
...
def test_ACCESS_ERROR_channel_user_not_owner(three_messages_two_users):
    response = requests.post(URL_message_pin, json={"token": three_messages_two_users['second_token'], 
...
def test_ACCESS_ERROR_dm_user_not_owner(three_dm_messages, second_user_flask):
    response = requests.post(URL_message_pin, json={"token": second_user_flask['token'], 
    "message_id" : three_dm_messages['message1_ID']})

    assert response.status_code == ACCESS_ERROR

def test_ACCESS_ERROR_invalid_token(three_messages):
    response = requests.post(URL_message_pin, json={"token": (three_messages['token'] + '1'), 
    "message_id" : three_messages['message1_ID']})

    assert response.status_code == ACCESS_ERROR

def test_VALID_STATUS_channel_message_pin(three_messages):
    response = requests.post(URL_message_pin, json={"token": three_messages['token'], 
    "message_id" : three_messages['message1_ID']})
    ...
    assert messages['is_pinned'] == True

def test_VALID_STATUS_dm_message_pin(three_dm_messages):
