...
from tests.routes_test import URL_message_react, URL_message_unreact
...
def test_INPUT_ERROR_invalid_message_id(three_messages):
    ...
    response_1 = requests.post(URL_message_unreact, json={"token": three_messages['token'], 
...
def test_INPUT_ERROR_user_not_in_channel(three_messages, third_user_flask):
    ...
    response_1 = requests.post(URL_message_unreact, json={"token": third_user_flask['token'], 
    "message_id" : three_messages['message1_ID'], "react_id": THUMBS_UP_REACT})
...
def test_INPUT_ERROR_invalid_react_id(three_messages):
    ...
    response = requests.post(URL_message_unreact, json={"token": three_messages['token'], 
    "message_id" : three_messages['message1_ID'], "react_id": 0})

    assert response.status_code == INPUT_ERROR

def test_INPUT_ERROR_message_not_already_reacted(three_messages):
    response = requests.post(URL_message_react, json={"token": three_messages['token'], 
    "message_id" : three_messages['message1_ID'], "react_id": THUMBS_UP_REACT})
    response_1 = requests.post(URL_message_unreact, json={"token": three_messages['token'], 
    "message_id" : three_messages['message1_ID'], "react_id": THUMBS_UP_REACT})
    response_2 = requests.post(URL_message_unreact, json={"token": three_messages['token'], 
    "message_id" : three_messages['message1_ID'], "react_id": THUMBS_UP_REACT})
...
def test_ACCESS_ERROR_invalid_token(three_messages):
    response = requests.post(URL_message_unreact, json={"token": (three_messages['token'] + '1'), 
    "message_id" : three_messages['message1_ID'], "react_id": THUMBS_UP_REACT})

    assert response.status_code == ACCESS_ERROR

def test_VALID_STATUS_channel_message_unreact(three_messages):
    response = requests.post(URL_message_react, json={"token": three_messages['token'], 
    "message_id" : three_messages['message1_ID'], "react_id": THUMBS_UP_REACT})
    ...
    assert messages['reacts'][0]['is_this_user_reacted'] == False

def test_VALID_STATUS_dm_message_unreact(three_dm_messages):
    response = requests.post(URL_message_react, json={"token": three_dm_messages['first_token'], 
    "message_id" : three_dm_messages['message1_ID'], "react_id": THUMBS_UP_REACT})

    assert response.status_code == VALID_STATUS

    response_1 = requests.post(URL_message_unreact, json={"token": three_dm_messages['first_token'], 
    "message_id" : three_dm_messages['message1_ID'], "react_id": THUMBS_UP_REACT})
