...
def test_VALID_STATUS_valid_request(first_user_flask):
    response = requests.put(URL_user_profile_setemail, json={"token": first_user_flask['token'], 
...
def test_ACCESS_ERROR_invalid_token(first_user_flask):
    response = requests.put(URL_user_profile_setemail, json={"token": first_user_flask['token'] + '1', 
    "email": 'setemail@gmail.com'})
    assert response.status_code == ACCESS_ERROR

def test_INPUT_ERROR_invalid_email(first_user_flask):
    response = requests.put(URL_user_profile_setemail, json={"token": first_user_flask['token'], 
    "email": 'bademail'})
    assert response.status_code == INPUT_ERROR

def test_INPUT_ERROR_duplicate_email(first_user_flask):
