...
from tests.routes_test import  URL_clear, URL_channel_invite
...
def test_INPUT_ERROR_invalid_channel_id(first_channel_public_flask, second_user_flask):
    response = requests.post(URL_channel_invite, json={"token": first_channel_public_flask['token'], 
    "channel_id": (first_channel_public_flask['channel_id'] + 1), "u_id": second_user_flask['auth_user_id']})
    assert response.status_code == INPUT_ERROR

def test_INPUT_ERROR_invalid_user_id(first_channel_public_flask, second_user_flask):
    response = requests.post(URL_channel_invite, json={"token": first_channel_public_flask['token'], 
    "channel_id": first_channel_public_flask['channel_id'], "u_id": (second_user_flask['auth_user_id']
     + first_channel_public_flask['user_id'] + 1)})
    assert response.status_code == INPUT_ERROR

def test_INPUT_ERROR_user_in_channel(first_channel_public_flask, second_user_flask):
...
def test_ACSESS_ERROR_invalid_token(first_channel_public_flask, second_user_flask):
    response = requests.post(URL_channel_invite, json={"token": (first_channel_public_flask['token'] 
    + second_user_flask['token']), 
    "channel_id": first_channel_public_flask['channel_id'], "u_id": second_user_flask['auth_user_id']})
    assert response.status_code == ACCESS_ERROR

def test_ACSESS_ERROR_token_user_not_in_channel(first_channel_public_flask, second_user_flask):
    response = requests.post(URL_channel_invite, json={"token": second_user_flask['token'], 
    "channel_id": first_channel_public_flask['channel_id'], "u_id": second_user_flask['auth_user_id']})
    assert response.status_code == ACCESS_ERROR

def test_VALID_STATUS_user_invite(first_channel_public_flask, second_user_flask):
    response_channel_invite = requests.post(URL_channel_invite, json={"token": first_channel_public_flask['token'], 
    "channel_id": first_channel_public_flask['channel_id'], "u_id": second_user_flask['auth_user_id']})
    ...
    assert response_channel_invite.status_code == VALID_STATUS
